...
always be resumed by any tool — not only by this app. Everything here reads real
files.
"""
...
def list_checkpoints(run_dir: str | Path) -> list[dict[str, Any]]:
    ...
    for child in root.iterdir():
        if not child.is_dir() or not child.name.startswith(CHECKPOINT_RE):
            continue
        try:
            step = int(child.name[len(CHECKPOINT_RE):])
        except ValueError:
            continue
